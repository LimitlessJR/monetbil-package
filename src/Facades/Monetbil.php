...
namespace Tchdev\Monetbil\Facades;
...
abstract class Monetbil extends Facade
{
    ...
    public static function js($autoopen = false)
    {
        ...
        if (self::MONETBIL_WIDGET_VERSION_V2 == self::getWidgetVersion()) {
            $js = '<script type="text/javascript" src="vendor/monetbil/assets/js/monetbil' . $auto . '.min.js?t=' . time() . '"></script>';
        } else {
            $js = '<script type="text/javascript" src="vendor/monetbil/assets/js/monetbil-mobile-payments' . $auto . '.js?t=' . time() . '"></script>';
        }
        return $js;
    }
...
Monetbil::setServiceKey(config('monetbil.key'));
Monetbil::setServiceSecret(config('monetbil.secret'));

// To use responsive widget, set version to 'v2.1'
Monetbil::setWidgetVersion(config('monetbil.version'));
